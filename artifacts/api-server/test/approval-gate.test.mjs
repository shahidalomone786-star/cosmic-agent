...
test("approval is required and binds to the exact proposal version", () => {
  ...
  const approval = approveProposal(first, undefined, "authenticated-user", "task-1");
  assert.equal(assertApproval(approval.approvalId, first, undefined, "task-1").proposalVersion, proposalVersion(first));
  assert.throws(() => assertApproval(approval.approvalId, proposal("two"), undefined, "task-1"), /approval|stale/i);
});
